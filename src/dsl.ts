...
export class Workflow {
  ...
  theme_allocation(options: {
    ...
    if (
      inp !== 'dataset' &&
      !this.sources[inp] &&
      !this.processes.some(p => p.id === inp)
    ) {
  ...
  static fromFile(filePath: string): Workflow {
    ...
    for (const step of pipeline) {
      if (!step || typeof step !== 'object' || Array.isArray(step) || Object.keys(step).length !== 1) {
  ...
  async run(dataset?: any, options: { client?: CoreClient; fast?: boolean } = {}): Promise<any> {
    ...
    // simple linear mode
    const analyzer = new Analyzer({ dataset, processes: this.processes, client: client as CoreClient, fast })
    return analyzer.run()
  }
  ...
  private async runDsl(client?: CoreClient, fast?: boolean): Promise<any> {
    ...
    for (const proc of this.processes) {
      ...
      const ctx: any = { client: c, fast: proc['fast'] ?? fast ?? false, sources: ctxSources, results }
      ...
      switch (orig) {
        ...
        case SentimentProcess.id:
          ...
          wrapped = new SentimentResult({ results: nested, requestId: flatRaw.requestId }, flatTexts)
        ...
        case ThemeExtraction.id:
          wrapped = new ThemeExtractionResult(raw, ctx.dataset, (proc as ThemeExtraction).themes ?? [])
